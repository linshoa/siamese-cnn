...
def spatio_temporal(location_left, location_right, time_left, time_right):
    ...
    diff_concat = tf.concat([spatio_diff, location_diff])
    fc_1 = slim.fully_connected(slim.flatten(diff_concat), 8)
    fc_2 = slim.fully_connected(fc_1, 16, activation_fn=slim.nn.sigmoid)
    fc_reshape = tf.reshape(fc_2, [config.BATCH_SIZE, 1, 16])
    return fc_reshape
...
def contrastive_loss(_left, _right, _label_input, _info_left_location, _info_right_location, _info_left_time, _info_right_time):
    with tf.name_scope('output'):
        ...
        fc_out = slim.fully_connected(slim.flatten(diff_feature), 16, activation_fn=None)
        _inner_product = tf.reshape(fc_out, [config.BATCH_SIZE, 1, 16])

        feature_spatio_temporal = spatio_temporal(_info_left_location, _info_right_location, _info_left_time, _info_right_time)

        concat_visual_spatio_temporal = np.concatenate([_inner_product, feature_spatio_temporal], axis=1)
        ...
        # todo next concat the two feature. while the visual feature need to scale to the same feature.
        # 输出 [1,0] or [0, 1]
    ...
    return __loss_out, _loss, final_feature_out
...
if __name__ == '__main__':
    with tf.name_scope('input'):
        ...
        left_location = tf.placeholder(tf.float32, [None, 1, 1], name='left_location')
        right_location = tf.placeholder(tf.float32, [None, 1, 1], name='right_location')
        left_time = tf.placeholder(tf.float32, [None, 1, 1], name='left_time')
        right_time = tf.placeholder(tf.float32, [None, 1, 1], name='right_time')
    ...
    with tf.Session() as sess:
        ...
        loss, intermediate_loss, _inner_product = contrastive_loss(left_feature, right_feature, label_input, left_location, right_location, left_time, right_time)
        ...
        for i in range(config.EPOCH):
            ...
            _info_left_location = np.array(_info_left).transpose()[1]
            _info_left_time = np.array(_info_left).transpose()[2]
            _info_right_location = np.array(_info_right).transpose()[1]
            _info_right_time = np.array(_info_right).transpose()[2]
            __, _, sum_loss, not_sum_loss, summary_str = sess.run([_inner_product, train_step, loss,  intermediate_loss, merged], feed_dict={left: left_array, right: right_array, label_input: label_float32, left_location: _info_left_location, right_location: _info_right_location, left_time: _info_left_time, right_time: _info_right_time})
